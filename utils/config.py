...
def config(task, dataset, model, opt=None, framework=None):
    ...
    args.file = {
        ...
        # 'data_dir': data_dir+f"{task}/semeval/", # 数据路径
        'data_dir': data_dir+f"{task}/", # 数据路径
        # 'save_dir': root_dir+f'CheckPoints/{task}/{dataset}/', # 模型保存路径
        'save_dir': save_dir+f'/{task}/{dataset}/',
        'cache_dir': cache_dir+f'/{task}/{dataset}/', # 缓存路径, 加快加载

        'log': f'./logs/{dataset}/',
        'record': f'./records/{dataset}/',
    }
    sys.path.append(args.file['data_dir']) # 添加完整数据路径
    if not os.path.exists(args.file['log']): os.makedirs(args.file['log']) # 创建日志保存路径
    if not os.path.exists(args.file['record']): os.makedirs(args.file['record']) # 创建记录保存路径
    if not os.path.exists(args.file['save_dir']): os.makedirs(args.file['save_dir']) # 创建模型保存路径
